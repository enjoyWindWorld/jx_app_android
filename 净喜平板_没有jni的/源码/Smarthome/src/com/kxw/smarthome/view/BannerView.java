...
import intf.ViewPagerOnItemClickListener;

import java.util.List;

import com.kxw.smarthome.R;
import com.kxw.smarthome.adapter.BannerAdapter;
import com.kxw.smarthome.utils.DensityUtil;
...
public class BannerView extends RelativeLayout implements ViewPager.OnPageChangeListener, ViewPagerOnItemClickListener {
    //选中显示Indicator
    private int mSelectRes = R.drawable.icon_banner_ellipse;

    //非选中显示Indicator
    private int mUnSelcetRes = R.drawable.icon_banner_circle;
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        if (ToolsUtils.isEmpty(mDelayTime)) {
            mDelayTime = "3000";
        }
    ...
    public void onPageSelected(int position) {
        if (mIsCycle) {
            for (int i = 0; i < mLength; i++) {
                ...
                if (position == i + 1) {
                    childView.setBackgroundResource(R.drawable.icon_banner_ellipse);
                } else {
                    childView.setBackgroundResource(R.drawable.icon_banner_circle);
                }
            }
        } else {
            for (int i = 0; i < mLength; i++) {
