...
//		context.registerReceiver(wifiResultChange, labelIntentFilter);
		...
		registerReceiver(wifiResultChange, filter);
	}
...
//			WiFiConnectProgress.dismiss();
		...
		wifiListAdapter.setData(newWifList);
	}

	private void isConnectedWifi() {
		System.out.println("--- 0 showWifiList isConnectedWifi  ="+ connectedWifi.size());		
		is_connected_ll.setVisibility(View.VISIBLE);
		wifi_state_iv.setImageResource(R.drawable.wifi_connect_img);
		System.out.println("--- 1 showWifiList isConnectedWifi  ="+ connectedWifi.size());
...
//			WiFiConnectProgress.dismiss();
	...
	//从新的里面找原来连接过的，重新连接
	private void reConnectedWifi()
	{
		WiFiInfo wiFiInfo = DBUtils.getFirstData(WiFiInfo.class);
		if(wiFiInfo != null)
		{
			for (ScanResult scanResult : connectedWifi) {
				if(scanResult.SSID.equals(wiFiInfo.SSID))
				{
					// 此处加入连接wifi代码
					int netID = wifiUtils.CreateWifiInfo2(scanResult, wiFiInfo.getPwd());
					MyLogger.getInstance().e("Dialog---  netid = " + netID);
					boolean connect =  wifiUtils.ConnectToNetID(netID);
					if(connect){
						DBUtils.deleteAll(WiFiInfo.class);
						if(pwd != null && scanResult.SSID != null && scanResult.BSSID != null && scanResult.capabilities != null){
