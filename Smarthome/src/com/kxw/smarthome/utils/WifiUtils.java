...
public class WifiUtils {
	...
	public int CreateWifiInfo2(ScanResult wifiinfo, String pwd) {
		...
		MyLogger.getInstance().e("name = "+wifiinfo.SSID+" wifiinfo.BSSID = "+wifiinfo.BSSID+" pwd = "+pwd);
		if (config != null) {
	...
	public int CreateWifi(WiFiInfo wifiinfo) {
		WifiCipherType type;
		...
		WifiConfiguration config = CreateWifiInfo(wifiinfo.SSID,
				wifiinfo.BSSID, wifiinfo.pwd, type);
		MyLogger.getInstance().e("name = "+wifiinfo.SSID+" wifiinfo.BSSID = "+wifiinfo.BSSID+" pwd = "+wifiinfo.pwd);
		if (config != null) {
			return wifiManager.addNetwork(config);
		} else {
			return -1;
