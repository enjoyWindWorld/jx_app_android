...
public class MySubordinateOrderFragment extends RHBaseFragment {
...
//        mSwipeMenuRecyclerView.addItemDecoration(new ListViewDecoration());// 添加分割线。
     ...
     * 获取我的订单列表
     */
    public void getSubordinateOrderList() {
        getPreOrderData();
        ...
        dialog.show();
        orderDao.getSubordinateOrderListTask(StringUtil.isEmpty(ord_managerno) ? SesSharedReferences.getPartnerNumber(getActivity()) : ord_managerno, page + "", state, SesSharedReferences.getSafetyMark(getActivity()), new ResponseResult() {
            @Override
    ...
    void getPreOrderData() {
        if (page == 1) {
            ...
            map.put("username", StringUtil.isEmpty(ord_managerno) ? SesSharedReferences.getPartnerNumber(getActivity()) : ord_managerno);
            map.put("page", "1");
            map.put("state", state);
            map.put("safetyMark", SesSharedReferences.getSafetyMark(getActivity()));

            String js = dbManager.getUrlJsonData(Constant.USER_GET_SUB_ORD_WP + StringUtil.obj2JsonStr(map));
